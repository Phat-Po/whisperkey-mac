...
- **豆包模式 / 实时流式 ASR** — 边说边在底部 bar 出字；多句录音自动累积，录音结束后完整内容送入清理整理
- **自动粘贴** 到当前应用
- **VoiceInput 胶囊浮层** — 紧凑低干扰的录音/转录/结果视觉回馈

### 🧼 可选 AI 后处理
- **语音清理（Voice Cleanup）** — 把散乱口语转换成结构化的可执行指令（主题 / 目标 / 任务 / 需求 / 限制 / 偏好 / 输入 / 输出 / 备注），去除口头禅、删除重复、自动处理自我修正
...
免费包会标注为 `free-selfsigned`，用的是本地自签名证书签名。它没有 Apple 公证，所以 macOS 首次启动时可能需要到 **系统设置 → 隐私与安全性 → 仍要打开** 手动确认。因为每个版本都用同一份证书签名，权限授权（辅助功能/输入监控/麦克风）会跨版本保留，升级后一般不需要重新授权。
...
| **Doubao Mode (Real-time)** | 使用豆包/火山引擎流式 ASR 替代本地 Whisper。边说边在底部 bar 出字；多句录音自动累积完整内容，录音结束后统一清理整理。 | 快速听写、实时笔记、低延迟优先场景 | 8 秒 |
